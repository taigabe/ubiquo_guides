...
modular structure in a totally dynamic way, with some advanced capabilities like
drag & drop widgets, different block configurations or page templates.

The main features are:

* Create Pages with its own manager.
* Define the parts of a page with blocks
* Chose a specific template for each page with its own block structure to set widgets in groups.
* Assign widgets to different blocks of a page as individual items.
...
If you are designing a publicly accessible website, it will be necessary to create public pages to show contents created inside the different Ubiquo data managers.
Each page in public part should have its corresponding page on ubiquo. The Pages manager allows ubiquo users to create pages, design, and finally publish them.
...
!images/ubiquo_design/form.png(Creating Pages)!

INFO: Once created a page, we cannot change his related page template from Ubiquo. It is necessary to delete and create page again or change it from the console.
...
!images/ubiquo_design/design.png(Template preview)!
...
UbiquoDesign have its own cache framework, aimed to cache entire widgets with arbitrary policies defined by the programmer.

h4. How it works

There is only one step that the programmer must follow to define policies for caching widgets, and this is to fill the config/initializers/design_cache.rb file with the desired policies.
