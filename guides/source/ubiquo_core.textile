...
Settings offers a place where store configuration variables

It allows the agrupation of values in contexts using _context_ method.
...
contexted_value = Settings[:context][:key]
contexted_value = Settings[:context].key
</ruby>

h4. Creating a new normal value

You can create new values for the setting. Before accesing a setting, its required to register it with _add_ or the type specific methods we will cover later.
...
Or using the extended format, specifically intented to allow runtime modifications
...
when a key is added you can override its value assigning another or using with _set_
method:

<ruby>
Settings[:context][:key] = normal_value

Settings.set(:key, 'normal_value')
Settings[:context].set(:key, normal_value)
</ruby>
...
h4. Runtime changes to settings

Settings can be configured to allow them to be changed by end-users on runtime.
To achieve this goal, a set of connectors has been stablished to keep settings
