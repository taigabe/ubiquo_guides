...
NOTE: For convention, the attribute name should be always plural no matter if it contains a single element (size option is 1). The media_attachment accessor always returns an array, so naming it in plural will remind you when you use this association in the code.

h5. Minimum amount of assets

As we have seen, :size allows you to set the maximum amount of related assets.
...
This option creates a validation roughly equivalent to:

<ruby>
  validates_presence_of :images_asset_relations
</ruby>

So if you need to customize it in your model, don't use the :required option and
add the one that suits you in your model.
