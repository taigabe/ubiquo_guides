...
h3. Filter your elements with category_sets filters


h4. render_category_filter and filter_category_info

There are available 'render_category_filter' and
'filter_category_info' helpers and you can add filters
in your scaffolds searching your records by categories.
...
def painting_filters_info(params)
  filters = []
  filters << filter_category_info(params, :set => 'colors')
  build_filter_info(*filters)
end

def painting_filters(url_for_options = {})
  filters = []
  filters << render_category_filter(url_for_options, :set => 'colors')
  filters.join
end
...
Ex: render_category_filter(url_for_options, :set => 'colors', **:caption => 'Colors filter example'**)
...
By default, the category_selector helper is only in UbiquoAreaController. If you want to use ubiquo_categories for your public forms, you can do it by including the following module:
