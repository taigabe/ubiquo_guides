...
|_.File                                           |_.Purpose|
| app/views/ubiquo/books/index.html.erb           | A view to display a index of all books |
...
Imagine now that you want to add an _authors_ ubiquo resource. If you need it to appear on the same tab that _books_, edit the books navigation file +app/views/navigators/_books_navlinks.html.erb+ and add a _authors_ link:
...
With new ubiquo version, we have reduce a code generated in index view and move it to helper. With this form, if we need do any change, we work with helper usually instead of view.
...
The +books_list+ method render standard partial located in +ubiquo_core plugin+. Let's see its parameters:
...
h3. Applying filters to model and controllers
