...
This will currently install all the available plugins. You now have a working rails application install with ubiquo applied. When the installation is finished, it will prompt you about the next steps to do (configure a database, initialize it and create an admin account).When you have everything in place, just fire the server
