...
<shell>
$ rails new_app -m http://ubiquo.gnuine.loc
</shell>

This will currently install all the available plugins. During the installation, it will ask you a couple of questions (application name, and if you want to install these guides). But that's all. You now have a working rails install with ubiquo applied. It already executed the ubiquo:install and the database-related tasks. So, with everything in place, just fire the server
...
And now visit http://localhost:3000/ubiquo. Enter with the gnuine/gnuine credentials (the default superadmin), and you will see the default Ubiquo interface. Now, to fill it with your own stuff, it's time to start using the "Ubiquo Scaffold guide":ubiquo_scaffold.html
