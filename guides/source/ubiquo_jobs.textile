...
the best way to do so is to implement your own manager. A nice rails-like way to do that is include them i the +lib/+ folder of your ubiquo project.
...
to reimplement the +self.job_class+ to return your own kind of job:
...
However, implement a new job class is not always the best choice. For this special case, the default +UbiquoJob+ class provides a special member which stores the
